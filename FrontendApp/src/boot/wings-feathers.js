...
export default (host, services) => {
  ...
  app
    ...
    .configure(
      auth({
        ...
        storage: process.env.CLIENT ? window.localStorage : undefined // Passing a WebStorage-compatible object to enable automatic storage on the client.
      })
    );
  ...
  app.serve = async (serviceName, defaultQuery) => {
    ...
    service.on(`patched`, message => {
      ...
      if (index > -1 && (!channelLength || inChannel)) {
        // meron sa data and meron sa channel
        service.data[index] = Object.assign(service.data[index], message);
        event.emit("dataChange", service.data);
      } else if (!channelLength || inChannel) {
        // wala sa data pero meron sa channel, so add it
        service.data.unshift(message);
        event.emit("dataChange", service.data);
      } else if (index > -1 && !(!channelLength || inChannel)) {
        // meron sa data pero WALA sa channel, so remove it
        service.data.splice(index, 1);
        event.emit("dataChange", service.data);
      }
    });
