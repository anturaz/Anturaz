export default async ({ app, router, Vue }) => {
  ...
  Vue.prototype.$StoreUserLogin = async function(email, password) {
    ...
    try {
      ...
      var logged_in_user = await this.$dbCon.services.users.get(payLoad.userId);
      ...
      } else if (logged_in_user.system_user_type == "Employee") {
        var store_employee = await this.$dbCon.service("store-users").find({
          query: {
