<template>
  <div>
    ...
    <q-dialog v-model="opened" persistent>
      <q-card
        :style="$q.screen.lt.sm ? '' : 'min-width: 700px; max-width: 80vw;'"
      >
        ...
        <q-card-section style="max-height: 50vh" class="scroll q-gutter-sm">
          ...
          <div :class="$q.screen.lt.sm ? '' : 'row'">
            <div class="col-6 items-center">
              ...
              <q-select
                v-model="updated.user"
                :options="['Store Owner', 'Shopper', 'All']"
                label="User"
              />
              <br />
              <q-input
                outlined
                v-model="updated.content"
                type="textarea"
                label="Content"
              />
            </div>
            ...
            <div class="col-5 items-center ">
              ...
              <q-date
                v-model="updated_until"
                :options="date => date >= $formatDateForPicker(new Date())"
                minimal
              />
            </div>
          </div>
  ...
  methods: {
    ...
    save: function() {
      ...
      this.$dbCon
        ...
        .update(this.updated._id, this.updated);
      this.opened = false;
    },
    close: function() {
