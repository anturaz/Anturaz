    ...
    <div class=" row text-black">
      ...
      <div class="row no-wrap">
        <div
          ...
          v-for="store in selectedData"
          :key="store._id"
          @click="$router.push('/StoreDetails?store=' + store._id)"
          style="cursor:pointer; height: 150px; width: 220px"
        >
      ...
      <div class="float-right" style="cursor:pointer; height: 100px">
        <q-btn
          @click="moveCurrentIndex(2)"
          flat
          v-show="currentIndex + 6 <= data.length"
        >
          <q-icon name="fas fa-angle-right" style="height:150px;" />
        </q-btn>
      </div>
...
export default {
  ...
  computed: {
    // readonly
    selectedData() {
      let selArr = this.data.slice(this.currentIndex, this.currentIndex + 8);
      return selArr;
    }
  },
  methods: {
    moveCurrentIndex(mode) {
      if (mode == 1) {
        ...
        if (this.data.length > this.currentIndex) this.currentIndex += 8;
      }
    }
  },

  async mounted() {
    const currentIsoString = new Date(Date.now()).toISOString();
