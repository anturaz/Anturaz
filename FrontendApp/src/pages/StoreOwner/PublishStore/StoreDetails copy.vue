...
export default {
  ...
  methods: {
    ...
    storeFuelDelivery: async function() {
      // this.$dbCon
      ...
      //     console.log(this.selected_municipalities);
      //   });
      const query = {
        query: {
          store_id: this.store._id
      ...
      try {
        const response = this.$dbCon.service("store-fuel-delivery").find(query);
        console.log(response);
        this.store_fuel_delivery_data = data;
      } catch (e) {}
    },
    ...
    save: async function() {
      ...
      this.$dbCon
        ...
        .then(async result => {
          const fuelDeliveryData = result.data[0];

          const newDeliveryArr = this.selected_municipalities.map(
            municipality => {
              const munData = fuelDeliveryData.fuel_delivery.find(
                fuelDel => fuelDel.location === municipality.location
              );
              return munData
                ? {
                    location: munData.location,
                    province: munData.province,
                    fuel_transpo: munData.fuel_transpo,
                    delivery: munData.delivery
                  }
                : municipality;
            }
          );
          ...
          const newData = {
            ...fuelDeliveryData,
            fuel_delivery: newDeliveryArr
          };

          await this.$dbCon
            .service("store-fuel-delivery")
            .patch(fuelDeliveryData._id, newData)
            .then(res => {});
        });
    },
    back: async function() {
  ...
  watch: {
    ...
    "store.market_areas": {
      handler: function(newVal) {
        // Agusan Del Norte
        const selectedMun = this.$municipalities.filter(({ province }) =>
          newVal.includes(province)
        );

        this.selected_municipalities = selectedMun.map(municipality => {
          return {
