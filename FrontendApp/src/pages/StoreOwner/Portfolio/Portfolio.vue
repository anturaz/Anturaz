<template>
  <div class="row">
    ...
    <br />
    <input type="file" ref="uploader" multiple v-show="false" @change="onImageSelected" />
    <div class="col-md-8 col-xs-12">
      <q-card :class="!$q.screen.lt.md? 'q-ml-md':'q-mt-md'" bordered flat>
        <q-card-section>
          ...
          <div
            v-if="compute_used==total_storage && total_storage==0.5"
          >You have consumed your free 5MB storage.</div>
          <q-linear-progress stripe style="height: 10px" :value="compute_used/total_storage" />
        </q-card-section>
        <q-card-section class="row justify-end">
          <AvailStorageSpace :class="$q.screen.lt.md?'full-width q-pb-sm':''" />
          <q-btn
          ...
          <q-btn
            ...
            :disable="store.gallery.length==0"
            unelevated
            @click="isDeletingMultiple=false; deleteMultiple()"
            :class="$q.screen.lt.md?'full-width':''"
          />
        </q-card-section>
        <q-card-section
        ...
        >
          ...
          <div v-for="(galleryFile,index) in store.gallery" v-bind:key="index">
            <q-card v-if="galleryFile.type=='image'" flat bordered style="width:130px;height:130px">
              <q-btn
                flat
                style="width:100%;height:100%"
                @click="isDeletingMultiple? selectToDelete(galleryFile):viewImage(galleryFile,index)"
              >
                <q-checkbox
                  v-if="isDeletingMultiple"
                  :value="toDelete.includes(galleryFile)"
                  @click.native="isDeletingMultiple? selectToDelete(galleryFile):viewImage(galleryFile,index)"
            ...
            >
              <q-btn
                flat
                style="width:100%;height:100%"
                @click.native="isDeletingMultiple? selectToDelete(galleryFile):viewVideo(galleryFile,index)"
              >
                <q-checkbox
                  v-if="isDeletingMultiple"
                  :value="toDelete.includes(galleryFile)"
                ...
                <q-img
                  class="col"
                  :src="'https://img.youtube.com/vi/'+getYTID(galleryFile.link)+'/maxresdefault.jpg'"
                  :ratio="1"
                />
              </q-btn>
            </q-card>
      ...
      <q-card style="width:300px">
        ...
        <q-card-section>
          ...
          <div class="row no-wrap items-center">
            <div class="col text-h7 ellipsis q-pt-md">{{$formatDate(current_image.createdAt)}}</div>
            <div class="col-auto text-grey q-pt-md">
              <q-icon name="storage" />
              {{current_image.size}} MB
            </div>
          </div>
          <q-btn dense outline label="close" class="col-12 full-width q-mt-lg" v-close-popup />
        </q-card-section>
      </q-card>
    </q-dialog>
    <q-dialog v-model="viewVid">
      <q-card>
        ...
        <q-card-section>
          ...
          />
          <div class="row no-wrap items-center">
            <div class="col text-h7 ellipsis q-pt-md">{{$formatDate(current_video.createdAt)}}</div>
            <div class="col-auto text-grey q-pt-md">
              <q-icon name="storage" />
      ...
      <q-card>
        <q-card-section class="row items-center no-wrap">
          <q-list separator bordered style="width:100%">
            <q-item clickable v-ripple @click="$refs.uploader.click(), upload=false">
              <q-item-section avatar>
                <q-btn flat round icon="insert_photo" />
              </q-item-section>
              <q-item-section>Upload Picture</q-item-section>
            </q-item>
            <q-item clickable @click="addVideoLink(), upload=false" v-ripple>
              <q-item-section avatar>
                <q-btn flat round icon="link" />
              </q-item-section>
              <q-item-section style="color:primary">Add Youtube Video Link</q-item-section>
  ...
  methods: {
    ...
    uploadFile: async function (x) {
      ...
      if (["image/jpeg", "image/png"].includes(x.type)) {
        var fileLink = await this.$uploadFile(x);
