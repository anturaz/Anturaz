<template>
  <div>
    <q-card flat class="q-pb-md" align="center" v-if="$route.path=='/StoreOwner/PublishStore'">
      <q-card-section class="row">
        <q-icon class="col-12" name="verified_user" color="primary" size="100px" />
        <div class="col-12 text-h4 text-bold text-primary">&#8369;499.00/month</div>
        ...
        <div class="col-12 q-pt-lg" align="center">
          ...
          <div class="text-h6">
            <q-icon name="check" color="primary" />Store will be recommended on search results
          </div>
        </div>
      </q-card-section>
      <q-card-section class="q-pl-md-xl q-pr-md-xl row justify-center" v-if="ads_status!='PENDING'">
        ...
        <div class="col-md-5">
          <table width="100%">
            ...
            <tr>
              ...
              <td align="right">{{advertise_per_month*months_of_advertisment}}</td>
            </tr>
          </table>
        </div>
        <div class="col-12" />
    ...
    <q-card
      flat
      bordered
      v-if="$route.path=='/StoreOwner/PublishStore' && ads_status=='PENDING' || $route.path=='/StoreOwner'"
    >
      <q-card-section align="right">
        <div v-if="ads_status=='NONE'">
          <q-btn color="primary" label="Advertise Store" @click="StoreAdvertismentPlan=true" />
        </div>
      ...
      <q-card-section v-if="data.subscriptions.length!=0">
        <q-markup-table flat>
          <thead>
            <tr>
          ...
          <tbody>
            <tr v-for="(subscription,index) in data.subscriptions" v-bind:key="index">
              <td class="text-left">{{index+1}}</td>
              <td class="text-center">{{subscription.months}} Month{{subscription.months==1?'':'s'}}</td>
    ...
    <q-dialog v-if="data.subscriptions.length!=0" v-model="edit_advertise_store">
      <q-card>
        <q-card-section>
        ...
        <q-card-section>
          <table width="100%">
            ...
            <tr>
              ...
              <td align="right" style="width:30px;">
                <q-input
                  v-model="data.subscriptions[data.subscriptions.length-1].months"
                  ref="input_months_of_advertisment"
                  type="number"
                  @blur="months_of_advertisment_change()"
                />
            ...
            <tr>
              ...
              <td
                align="right"
              >{{data.subscriptions[data.subscriptions.length-1].months*advertise_per_month}}</td>
            </tr>
          </table>
        </q-card-section>
        <q-card-section class="q-gutter-sm">
          <q-btn class="full-width" color="primary" label="Pay via Bank Deposit" @click="edit" />
