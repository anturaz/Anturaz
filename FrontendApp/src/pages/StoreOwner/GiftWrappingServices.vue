    ...
    <div class="col-12">
      ...
      <!-- <q-list bordered>
        <q-item
          clickable
          v-ripple
          @click="editCharge('small', data.giftwrapping.charges.small)"
        >
          <q-item-section>Small</q-item-section>

          <q-item-section avatar>{{
            data.giftwrapping.charges.small == "" ||
            data.giftwrapping.charges.small == "0"
              ? "Free"
              : $prettyMoney(data.giftwrapping.charges.small)
          }}</q-item-section>
        </q-item>
        <q-separator />
        <q-item
          clickable
          v-ripple
          @click="editCharge('medium', data.giftwrapping.charges.medium)"
        >
          <q-item-section>Medium</q-item-section>
          <q-item-section avatar>{{
            data.giftwrapping.charges.medium == "" ||
            data.giftwrapping.charges.medium == "0"
              ? "Free"
              : $prettyMoney(data.giftwrapping.charges.medium)
          }}</q-item-section>
        </q-item>
        <q-separator />
        <q-item
          clickable
          v-ripple
          @click="editCharge('large', data.giftwrapping.charges.large)"
        >
          <q-item-section>Large</q-item-section>
          <q-item-section avatar>{{
            data.giftwrapping.charges.large == "" ||
            data.giftwrapping.charges.large == "0"
              ? "Free"
              : $prettyMoney(data.giftwrapping.charges.large)
          }}</q-item-section>
        </q-item>
        <q-separator />
        <q-item
          ...
          v-ripple
          @click="
            editCharge('extra_large', data.giftwrapping.charges.extra_large)
          "
        >
          <q-item-section>Extra Large</q-item-section>
          <q-item-section avatar>{{
            data.giftwrapping.charges.extra_large == "" ||
            data.giftwrapping.charges.extra_large == "0"
              ? "Free"
              : $prettyMoney(data.giftwrapping.charges.extra_large)
          }}</q-item-section>
        </q-item>
        <q-separator />
      </q-list> -->
