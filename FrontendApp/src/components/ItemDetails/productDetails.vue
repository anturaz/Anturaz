  ...
  <div>
    <q-item>
      ...
      <q-item-section side top>
        <span v-if="order.status == 'Done' || order.status == 'For Rating'">
          <span class="text-body1 text-primary text-title text-bold"
            >DONE&nbsp;</span
          >
        </span>
        <span
        ...
        <div v-if="this.payment['breakdown']">
          ...
          <q-item-section v-if="order.giftwrapping">
            <q-item-label caption lines="2">
              <span class="text-weight-bold">
                GiftWrap Color
              </span>
              ...
              <span
                :style="{
                  backgroundColor: order.giftwrapping_details.color,
                  width: '10px',
                  height: '10px',
          ...
          <!-- && order.status == status.ForReceiving && order.status == status.ForPickup && order.status == status.Done -->
          <q-item-section>
            <q-item-label caption lines="2">
              <div v-if="order.shipping_details != undefined">
                ...
                <br />
                <div v-if="order.status == 'For Receiving'">
                  <span class="text-weight-bold">Estimated Delivery Date:</span>
                  {{ $formatDate_DateOnly(order.shipping_details.eta) }}
                </div>
                <div v-else>
                  <span v-if="receivedLog">
                    <span class="text-weight-bold">Date Received:</span>
                    {{ $formatDate_DateOnly(receivedLog.date) }}
                    <span
                      v-if="receivedLog.event === 'Auto Accepted by system'"
                    >
                      (Auto-received)
                    </span>
                  </span>
                  <span v-else>No received log available.</span>
                </div>
              </div>
              <div v-else>
                <span v-if="receivedLog">
                  <span class="text-weight-bold">Date Received:</span>
                  {{ $formatDate_DateOnly(receivedLog.date) }}
                  <span v-if="receivedLog.event === 'Auto Accepted by system'">
                    (Auto-received)
                  </span>
                </span>
              </div>
...
export default {
  ...
  computed: {
    ...
    receivedLog() {
      if (!this.order || !this.order.logs) return null;

      return this.order.logs.find(
        log =>
          log.event === "Received" || log.event === "Auto Accepted by system"
      );
    },
    pickedUpLog() {
      if (!this.order || !this.order.logs) return null;
      return this.order.logs.find(log => log.event === "Picked up");
    }
  },
  mounted() {
    this.$dbCon
